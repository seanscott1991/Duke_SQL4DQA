...
-- 1.  Select the count of total rows in the charge table --
SELECT
	COUNT(*)
FROM
	charge;
...
-- 2. Select the count of total rows in the statement table --
SELECT
	COUNT(*)
FROM
	statement;
...
-- 3. Select all columns in the charge table and join in the statement date column from the statement table using a right join --
SELECT
	charge.*
    , statement.*
FROM
...
SELECT
	charge.*
    , statements.statement_dt
FROM
	charge 
    LEFT JOIN statements
		ON charge.statement_no = statements.statement_no;
