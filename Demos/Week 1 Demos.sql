...
FROM
 ...
 -- 4. Return the charge date column  with the alias 'Charge Date' from the charge table --   
SELECT
...
-- 6. Return the charge amount as well as the charge amount divided by 2 from the charge table --
SELECT
	charge_amt
     , charge_amt / 2
FROM
...
-- 7.  Return the charge amount rounded to 2 decimal places from the charge table --
SELECT
	ROUND(charge_amt, 2)
FROM
	charge;
...
-- 2. Return charge_amt, charge_dt and category_no from the charge table where category_no does not equal 4 and charge_amt is greater than 4000--
SELECT
	charge_amt
    , charge_dt
    , category_no
...
-- 2. Return the frequency of charges in each month from the charge table with a category_no of 1 or 3 --
SELECT 
	month(charge_dt)
    , count(*)
FROM
...
-- 3. Return the frequency of charges in each month from the charge table with a category_no of 1 or 3, but only return months with at least 40,000 charges --
SELECT 
	month(charge_dt)
    , count(*)
FROM
...
-- 1. Return all columns of the first 10 rows of the charge table --
SELECT
	*
FROM
	charge
...
-- 2. Return the month with the most charges along with how many charges that month had --
SELECT 
	month(charge_dt)
    , count(*)
FROM
