...
-- 1. Repeat 5.1.1 but have ranking reset each new day --
 SELECT
	charge_no
    , DATE(charge_dt)
    , charge_amt
...
-- 1. Return charge number, date, and amount for all of member 9766's category 9 charges --     
SELECT
	charge_no
    , charge_dt
    , charge_amt
...
SELECT
	...
	, DAILY_CHARGES
	,  ROUND(   
			((DAILY_CHARGES -  (LAG(DAILY_CHARGES, 1) OVER (ORDER BY CHARGE_DATE)) )  / (LAG(DAILY_CHARGES, 1) OVER (ORDER BY CHARGE_DATE))) *100
						, 2) AS PCT_CHANGE
FROM
	table1
;
